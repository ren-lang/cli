...
abort : Proxy -> () -> ()
abort (Proxy proxy) _ =
    FFI.call proxy "abort" [] (Json.Decode.succeed ())
        |> Maybe.withDefault ()


arch : Proxy -> Architecture
arch (Proxy proxy) =
    case FFI.get proxy "arch" Json.Decode.string of
        Just "arm" ->
            ARM
...
argv (Proxy proxy) =
    FFI.get proxy "argv" (Json.Decode.list Json.Decode.string)
        |> Maybe.withDefault []
...
cwd : Proxy -> () -> String
cwd (Proxy proxy) _ =
    FFI.call proxy "cwd" [] Json.Decode.string
        |> Maybe.withDefault ""


env : Proxy -> Dict String String
env (Proxy proxy) =
    FFI.get proxy "env" (Json.Decode.dict Json.Decode.string)
        |> Maybe.withDefault Dict.empty


exit : Proxy -> Int -> ()
exit (Proxy proxy) code =
    FFI.call proxy "exit" [ Json.Encode.int code ] (Json.Decode.succeed ())
        |> Maybe.withDefault ()


platform : Proxy -> Platform
platform (Proxy proxy) =
    case FFI.get proxy "platform" Json.Decode.string of
        Just "aix" ->
            AIX
