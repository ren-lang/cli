                ...
                if isPathProxy then
                    ...
                    Json.Decode.fail <|
                        "Uh oh, it looks like there was an internal error in "
                            ++ "the `FFI.Path.elm` module. Please open an issue at "
...
basename (Proxy proxy) path maybeExt =
    case maybeExt of
        Just ext ->
            FFI.call proxy "basename" [ Json.Encode.string path, Json.Encode.string ext ] Json.Decode.string
                |> Maybe.withDefault ""

        Nothing ->
            FFI.call proxy "basename" [ Json.Encode.string path ] Json.Decode.string
                |> Maybe.withDefault ""
...
dirname (Proxy proxy) path =
    FFI.call proxy "dirname" [ Json.Encode.string path ] Json.Decode.string
        |> Maybe.withDefault ""
...
isAbsolute (Proxy proxy) path =
    FFI.call proxy "isAbsolute" [ Json.Encode.string path ] Json.Decode.bool
...
join (Proxy proxy) paths =
    FFI.call proxy "join" (List.map Json.Encode.string paths) Json.Decode.string
        |> Maybe.withDefault ""
...
normalise (Proxy proxy) path =
    FFI.call proxy "normalize" [ Json.Encode.string path ] Json.Decode.string
        |> Maybe.withDefault ""
...
relative (Proxy proxy) from to =
    FFI.call proxy "relative" [ Json.Encode.string from, Json.Encode.string to ] Json.Decode.string
        |> Maybe.withDefault ""


resolve : Proxy -> List String -> String
resolve (Proxy proxy) paths =
    FFI.call proxy "resolve" (List.map Json.Encode.string paths) Json.Decode.string
        |> Maybe.withDefault ""


sep : Proxy -> String
sep (Proxy proxy) =
    FFI.get proxy "sep" Json.Decode.string
        |> Maybe.withDefault "/"
